...
{
    [Serializable]
    public partial class VarSet : DockForm
    {
        /// <summary>编辑前</summary>
        private List<DataCell> mSysVar = new List<DataCell>();
        public VarSet(List<DataCell> svar)
        {
            InitializeComponent();
            Set_MinMax();
            mSysVar = svar;
        ...
        private void VarSetForm_Load(object sender, EventArgs e) { }
        private void RefreshVarList()
        {
            if (mSysVar == null) return;
            if (mSysVar.Count == 0) return;
            int index = 0;
            dgv_VarList.Rows.Clear();
            foreach (DataCell data in mSysVar)
            {
        ...
        private void uibt_OK_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dgv_VarList.RowCount; ++i)
            {
                ...
                mSysVar.Add(new DataCell("全局变量", 0, DataType.单量, type, name, remark, "0", i, value, DataAtrr.全局变量));
            }
            Sol.mSol.mSysVar = mSysVar;
        }
