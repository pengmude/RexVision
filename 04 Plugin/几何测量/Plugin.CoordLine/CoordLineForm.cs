...
{
    public partial class CoordLineForm : FormBase
    {
        CoordLineObj mObj;
        Dictionary<string, RexView.ROI> mRoiList = new Dictionary<string, RexView.ROI>();
        public CoordLineForm(CoordLineObj Obj) : base(Obj)
        {
            mObj = Obj;
            InitializeComponent();
        }
        private void CoordLineForm_Load(object sender, EventArgs e)
        {
            InitImage();
        ...
        private void Hwindow_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (index.Length > 0)
            {
                mObj.mIntLine = (ROICoordLine)roi;
        ...
        public void InitLine()
        {
            ...
            {
                mWindowH.WindowH.genCoordLine(mObj.ModInfo.Name, mObj.TranLine.StartX, mObj.TranLine.StartY, mObj.TranLine.EndX, mObj.TranLine.EndY, ref mRoiList);
            }
            else if (mObj.mRImage != null)
            {
                mWindowH.WindowH.genCoordLine(mObj.ModInfo.Name, mObj.mRImage.Height / 4, mObj.mRImage.Height / 4, mObj.mRImage.Height / 4, mObj.mRImage.Width / 2, ref mRoiList);
            }
            else
            {
                mWindowH.WindowH.genCoordLine(mObj.ModInfo.Name, 500, 100, 500,500, ref mRoiList);
            }
        }
